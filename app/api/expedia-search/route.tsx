...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (authorization !== process.env.API_ENPOINT_KEY)
    ...
    if (!(name && latlong))
      return NextResponse.json('Unable to fetch hotel details.', { status: 404 });

    // return NextResponse.json({ id, name, latlong, guests, checkin, checkout });
    const expediaData = await expediaHotelSearch(
      id,
      name,
      latlong,
      guests.toString(),
